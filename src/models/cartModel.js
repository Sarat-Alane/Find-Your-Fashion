...
const getCartItems=async(custid)=>{
    ...
    try {
        connection = await pool.getConnection();
        const productquery=`select qty,category,wear,filename,price_per_item,wearsize,amount from cart,cart_items,images,filter where cart_items.pid=images.pid and cart_items.custid=? and filter.fid=(select fid from stock where stock.pid=cart_items.pid and cart_items.custid=?)`
        const [rows] = await connection.execute(productquery,[custid,custid]);
        
        return rows.map(row => ({qty:row.qty,category:row.category,wear:row.wear,filename:row.filename,price_per_item:row.price_per_item,wearsize:row.wearsize,amount:row.amount}))
